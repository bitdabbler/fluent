...
Copyright (c) 2009 The Go Authors. All rights reserved.
  - original source code:
    https://cs.opensource.google/go/go/+/refs/tags/go1.21.3:src/testing/slogtest/slogtest.go
  - license: https://github.com/golang/go/blob/master/LICENSE

Modifications:
- ignore zero time case:
...
func TestHandler(h slog.Handler, results func() []map[string]any) error {
	cases := []testCase{
		...
		// REPLACEMENT
		{
			// "ignore" is taken to mean omitted above, but many logging
			// protocols don't allow that, so we apply an alternative
			// interpretation of "ignore" - don't use the value provided (ignore
			// it), and use some other reasonable value (e.g. time.Now())
			explanation: withSource("a Handler should ignore a zero Record.Time"),
			f: func(l *slog.Logger) {
				l.Info("msg", "k", "v")
			},
			mod: func(r *slog.Record) { r.Time = time.Time{} },
			checks: []check{
				hasNotAttr(slog.TimeKey, time.Time{}),
			},
		},
		{
...
func hasNotAttr(key string, wantVal any) check {
	return func(m map[string]any) string {
		if s := hasKey(key)(m); s != "" {
			// okay if missing
			return ""
		...
		gotVal := m[key]
		if reflect.DeepEqual(gotVal, wantVal) {
			return fmt.Sprintf("%q: got %#v, did not want %#v", key, gotVal, wantVal)
		}
		return ""
	}
}
